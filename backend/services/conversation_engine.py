...
def update_state(session_id, student_text):
    ...
    if any(x in text for x in [
        ...
        "you’re safe", "i'm here", "i am here",
        "we can work through this", "it's okay"
    ]):
        state["trust"] += 10
        state["engagement"] += 5
        state["distress"] -= 5
    ...
    # ✅ BAD RESPONSES
    if any(x in text for x in [
        "just relax", "don't worry", "calm down",
    ...
    # ✅ GOOD QUESTIONS
    if any(x in text for x in [
        "how do you feel", "can you tell me more",
        "what does that feel like", "can you describe"
    ]):
        state["engagement"] += 5
        state["trust"] += 5

    # ✅ NEW: BEHAVIOURAL QUESTION DETECTION (MODALITY RULE)
    if any(x in text for x in [
        ...
        state["behaviour_explored"] = True  # ✅ NEW
